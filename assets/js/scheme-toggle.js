...
( function () {
	...
	var KEY = 'creative-agency-scheme';
	var root = document.documentElement;
	var strings = window.creativeAgencyScheme || { toDark: 'Switch to dark mode', toLight: 'Switch to light mode' };

	function isDark() {
		return root.classList.contains( 'creative-agency-dark' );
	}
	...
	function apply( dark ) {
		root.classList.toggle( 'creative-agency-dark', dark );
	...
	function init() {
		...
		// started.
		var buttons = document.querySelectorAll( '.creative-agency-scheme-toggle .wp-block-button__link' );
		...
		try {
			if ( ! localStorage.getItem( KEY ) && window.matchMedia ) {
				window.matchMedia( '(prefers-color-scheme: dark)' ).addEventListener( 'change', function ( e ) {
					...
					root.classList.toggle( 'creative-agency-dark', e.matches );
					Array.prototype.forEach.call( buttons, label );
				} );
			}
		} catch ( e ) {}
