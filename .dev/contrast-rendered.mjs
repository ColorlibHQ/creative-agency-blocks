 ...
 *
 *   WP_URL=... CREATIVE_AGENCY_URL=/some-page/ node .dev/contrast-rendered.mjs
 *   CREATIVE_AGENCY_DARK=1 WP_URL=... CREATIVE_AGENCY_URL=/ node .dev/contrast-rendered.mjs
 *
...
const path = process.env.CREATIVE_AGENCY_URL || '/';
...
// CREATIVE_AGENCY_DARK=1 measures the same page with dark mode on, which is where the
// palette is lifted rather than replaced and is the likeliest place for a
// pairing to fall under AA.
if ( process.env.CREATIVE_AGENCY_DARK ) {
	await page.evaluate( () => {
		document.documentElement.classList.add( 'creative-agency-dark' );
	} );
...
const candidates = await page.evaluate( () => {
	...
	document.querySelectorAll( 'h1,h2,h3,h4,h5,h6,p,li,a,dt,dd,summary,label,button,span' ).forEach( ( el, i ) => {
		...
		// Pure decoration, marked as such: the giant pale "Projects" behind a
		// section title is aria-hidden on purpose and is meant to be barely
		// there (WCAG 1.4.3 exempts text that is part of a pure decoration).
		if ( el.closest( '[aria-hidden="true"]' ) ) {
			return;
		}

		// The brand tile is the site's logo: it shows the initial of the site
		// title and nothing else. WCAG 1.4.3 exempts logotypes.
		if ( el.closest( '.creative-agency-brand' ) ) {
			return;
		}
		...
		// The Quick Fact figures are 50px; the rest of the page is held to 4.5.
		const px = parseFloat( style.fontSize );
		const large = px >= 24 || ( px >= 18.66 && Number( style.fontWeight ) >= 700 );

		el.setAttribute( 'data-ca-cr', String( i ) );
		out.push( {
			i,
			need: large ? 3 : 4.5,
...
for ( const c of candidates ) {
	...
	onPhotos++;
	const handle = await page.$( `[data-ca-cr="${ c.i }"]` );
...
if ( findings.length ) {
	console.error( `${ findings.length } text nodes under AA (4.5:1, or 3:1 for large text) on ${ path } (${ onPhotos } measured on photographs)` );
...
console.log( `every text node on ${ path } meets AA — 4.5:1, 3:1 for large text (${ candidates.length } measured, ${ onPhotos } on photographs)` );
