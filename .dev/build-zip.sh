#!/usr/bin/env bash
#
# Build the distributable theme zip.
#
# Ships only what a site needs to run: the generators in .dev/, the repo's own
# CLAUDE.md and the git history stay behind. Theme Check must be run against
# THIS output, not the working tree, or it reports on the tooling as well.
#
# Usage:  bash .dev/build-zip.sh [outdir]
#
# Without an outdir it builds in a fresh mktemp directory (never a fixed /tmp
# name another run could be using) and prints the zip's path.
set -euo pipefail

here="$( cd "$( dirname "$0" )/.." && pwd )"
out="${1:-$( mktemp -d "${TMPDIR:-/tmp}/creative-agency-build.XXXXXX" )}"
name="creative-agency"

rm -rf "$out"
mkdir -p "$out/$name"

cd "$here"
rsync -a \
	--exclude '.git' \
	--exclude '.github' \
	--exclude '.dev' \
	--exclude 'CLAUDE.md' \
	--exclude '.gitignore' \
	--exclude '.DS_Store' \
	--exclude 'node_modules' \
	./ "$out/$name/"

cd "$out"
zip -rq "$name.zip" "$name"

echo "$out/$name.zip"
du -sh "$out/$name" "$out/$name.zip" | sed 's/^/  /'
