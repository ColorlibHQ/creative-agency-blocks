#!/usr/bin/env python3
"""
Fail on any `creative-agency-*` CSS selector that nothing in the theme emits.
...
That is how a contact form shipped in Unioncorp, the theme this toolchain
comes from: it was ported from Pato and kept Pato's word for its form in the
stylesheet while its PHP emitted a different one. Six rules — the wrapper, the
two-column grid, the actions row, the notices and the honeypot — matched
nothing, and the honeypot rendered in full view above the form.

Class names built at runtime count as emitted. PHP such as
`'creative-agency-field--' . esc_attr( $name )` or JavaScript such as
`'creative-agency-' + name` registers its literal prefix, and any selector starting
with that prefix is treated as live. Without that, `.creative-agency-field--message`
would be reported, although inc/contact-form.php builds it for the message field.
...
TOKEN = re.compile(r"creative-agency-[a-z0-9_-]+")
SELECTOR = re.compile(r"\.(creative-agency-[a-z0-9_-]+)")
# A runtime prefix is the creative-agency- token that ENDS a string literal and is then
# concatenated with a variable. It must not be required to START the literal:
#
#   class="creative-agency-field creative-agency-field--' . esc_attr( $name ) . '"
#
# begins at `class="`, so the first version of this pattern — which wanted a
# quote directly before the prefix — missed `creative-agency-field--` and reported the
# live `.creative-agency-field--message` as dead. Its own first run caught that.
PHP_PREFIX = re.compile(r"""(creative-agency-[a-z0-9_-]*)['"]\s*\.\s*(?:[a-z_]+\s*\(\s*)?\$""")
# 'creative-agency-' + name   /   `creative-agency-${name}`
JS_PREFIX = re.compile(r"""(creative-agency-[a-z0-9_-]*)(?:['"]\s*\+|\$\{)""")
...
REGISTERED = re.compile(r"""array\(\s*'(creative-agency-[a-z0-9-]+)'\s*,\s*__\(""")
...
def main():
    ...
    print("%d creative-agency-* selectors, %d emitted literally, dynamic prefixes: %s"
    ...
    # theme shipped — `creative-agency-card` on 30 groups, `creative-agency-ghost` on 10
    # buttons, `creative-agency-ticks` on 10 lists, and not one rule for any of them.
    ...
    # for another theme's vocabulary (Unioncorp's scheme.css once carried Pato's `is-style-card`).
    styled = {}
    for path, text in css_text.items():
