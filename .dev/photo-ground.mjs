 ...
 */
export async function sampleBehind( page, handle, pad = 6, { text = false, round = false } = {} ) {
	...
	await handle.evaluate( ( el ) => {
		el.dataset.caVisibility = el.style.visibility;
	...
	const restore = () => handle.evaluate( ( el ) => {
		el.style.visibility = el.dataset.caVisibility || '';
		delete el.dataset.caVisibility;
	} );
	...
	const interior = [];
	const ring = [];
	// A round control (the play button) is a circle, not its bounding square:
	// its ground is the annulus just outside the circle, and the corners of
	// the square are photograph the control never touches.
