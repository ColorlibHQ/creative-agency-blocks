...
// CREATIVE_AGENCY_PATTERNS says where the patterns actually are in that case.
const patterns = process.env.CREATIVE_AGENCY_PATTERNS
	? resolve( process.env.CREATIVE_AGENCY_PATTERNS )
...
// `<?php ... ?>` -> `CAPHP0`, and back again after the round trip.
...
function restore( content, snippets ) {
	return content.replace( /CAPHP(\d+)X/g, ( _, i ) => snippets[ Number( i ) ] );
}
