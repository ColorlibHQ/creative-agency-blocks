#!/usr/bin/env python3
"""Block markup helpers for Creative Agency's pattern generators.
...
def sp(slug):
    slug = str(slug)
    # "0" is not a preset but it is a legitimate value: sections that meet
    # edge to edge, and a row of photographs with no gutter between them.
    if slug == "0":
        return "0"
...
def image(slug, alt, ratio=None, scale="cover", style=None, rounded=None, lightbox=False, width=None,
          ext="webp", extra_class=None, size="large"):
    ...
    url = "<?php echo esc_url( get_theme_file_uri( 'assets/images/%s.%s' ) ); ?>" % (slug, ext)
    return (
        '<!-- wp:image%s -->\n'
...
def separator(style="wide", align=None):
    data = {"className": "is-style-" + style}
...
def group(inner, align=None, background=None, text=None, padding_y=None,
          ...
          padding=None, radius=None, anchor=None, shadow=None, vertical=None):
    ...
    if shadow:
        data.setdefault("style", {})["shadow"] = "var:preset|shadow|" + shadow
        css.append("box-shadow:var(--wp--preset--shadow--%s)" % shadow)
    ...
    if vertical:
        layout_data["verticalAlignment"] = vertical
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
    ...
    data["url"] = "CREATIVE_AGENCY_IMAGE"
    data["dimRatio"] = dim
    if gradient:
    ...
    url = "<?php echo esc_url( get_theme_file_uri( 'assets/images/%s.webp' ) ); ?>" % slug
    attr_json = attrs(data).replace('"CREATIVE_AGENCY_IMAGE"', '"%s"' % url)
