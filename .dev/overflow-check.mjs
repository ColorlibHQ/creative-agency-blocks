 ...
 * nothing else — and in Pato the reservation form's inputs were 34px wider than their
 ...
 *
 *   WP_URL=https://example.com CREATIVE_AGENCY_PATHS=/,/contact/ node .dev/overflow-check.mjs
 *
...
const paths = ( process.env.CREATIVE_AGENCY_PATHS || '/' ).split( ',' ).filter( Boolean );
const widths = ( process.env.CREATIVE_AGENCY_WIDTHS || '1400,1024,768,390' ).split( ',' ).map( Number );
