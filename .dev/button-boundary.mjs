 ...
 *
 *   WP_URL=https://colorlibhub.com/creative-agency node .dev/button-boundary.mjs
 *   CREATIVE_AGENCY_DARK=1 WP_URL=… node .dev/button-boundary.mjs
...
const dark = process.env.CREATIVE_AGENCY_DARK === '1';
const paths = ( process.env.PATHS || '/,/about/,/work/,/work/nookdesk/,/services/,/contact/,/blog/' ).split( ',' );
...
for ( const path of paths ) {
	...
	// EXTRA_CSS lets a fix be switched off to prove the check still catches
	// the fault it was added for.
	...
	const buttons = await page.evaluate( () => {
		return [ ...document.querySelectorAll( '.wp-block-button__link, .wp-element-button' ) ]
			...
			// Hidden until hover (a project's "View details" over its photograph)
			// is measured by nobody until it is seen, and then over the photograph.
			.filter( ( el ) => el.offsetWidth > 0 && 'hidden' !== getComputedStyle( el ).visibility && ! el.closest( '.creative-agency-scheme-toggle' ) )
			.map( ( el, i ) => {
				el.setAttribute( 'data-ca-bb', String( i ) );
				...
				// Computed radii keep their unit: `50%` is half the box, not 50px.
				const r = cs.borderTopLeftRadius;
				const radius = r.endsWith( '%' ) ? ( parseFloat( r ) / 100 ) * el.offsetWidth : parseFloat( r ) || 0;
				return {
					i,
					round: radius >= el.offsetWidth / 2 - 1 && Math.abs( el.offsetWidth - el.offsetHeight ) <= 1,
					// An icon-only submit inside a search field: the field's own
					// border is its boundary, as the dark mode switch's hit area is.
					icon: !! el.closest( '.wp-block-search__button-inside' ),
					// The Underlined link style is a text link on a yellow bar, not a
					// button shape: its label must read, and the bar is decoration.
					link: !! el.closest( '.is-style-creative-agency-underline' ),
	...
	for ( const b of buttons ) {
		...
		if ( b.onPhoto ) {
			onPhotos++;
			const handle = await page.$( `[data-ca-bb="${ b.i }"]` );
			const ground = handle ? await sampleBehind( page, handle, 6, { round: b.round } ) : null;
			if ( ! ground ) {
