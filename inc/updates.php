 ...
 * call our filter.
 */
const CREATIVE_AGENCY_UPDATE_HOST     = 'updates.colorlib.com';
const CREATIVE_AGENCY_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/creative-agency.json';
const CREATIVE_AGENCY_UPDATE_CACHE    = 'creative_agency_update_response';
 ...
 * @return bool
 */
function creative_agency_updates_enabled() {
	/**
	 * Filters whether Creative Agency checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'creative_agency_check_for_updates', true );
}
 ...
 * @return array
 */
function creative_agency_update_payload() {
	$payload = array(
		'theme'      => 'creative-agency',
		'version'    => CREATIVE_AGENCY_VERSION,
	 ...
	 */
	return apply_filters( 'creative_agency_update_payload', $payload );
}
 ...
 * @return array|null
 */
function creative_agency_fetch_update() {
	if ( ! creative_agency_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( CREATIVE_AGENCY_UPDATE_CACHE );
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( CREATIVE_AGENCY_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( CREATIVE_AGENCY_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function creative_agency_check_update( $update, $theme_data, $theme_stylesheet ) {
	if ( $update || 'creative-agency' !== $theme_stylesheet ) {
		return $update;
	}

	$release = creative_agency_fetch_update();

	if ( ! $release || version_compare( $release['version'], CREATIVE_AGENCY_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => CREATIVE_AGENCY_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/creative-agency/',
...
add_filter( 'update_themes_' . CREATIVE_AGENCY_UPDATE_HOST, 'creative_agency_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function creative_agency_flush_update_cache() {
	delete_site_transient( CREATIVE_AGENCY_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'creative_agency_flush_update_cache' );
add_action( 'after_switch_theme', 'creative_agency_flush_update_cache' );
 ...
 * their server.
 */
function creative_agency_updates_notice() {
	$screen = get_current_screen();

	if ( ! $screen || 'appearance_page_creative-agency-starter-sites' !== $screen->id ) {
		return;
	}
	...
	$payload = creative_agency_update_payload();

	// Creative Agency ships no companion plugin, so this speaks only for the theme. If one
	...
	$note = __( 'Creative Agency checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'creative-agency' );
	?>
	<p class="description creative-agency-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( CREATIVE_AGENCY_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>creative_agency_check_for_updates</code>'
		);
		?>
	</p>
	<style>.creative-agency-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
