 ...
 * WooCommerce support.
 *
 * A studio that sells templates, prints or workshops ends up with
 ...
 */
function creative_agency_woocommerce_setup() {
...
add_action( 'after_setup_theme', 'creative_agency_woocommerce_setup' );
 ...
 */
function creative_agency_woocommerce_styles() {
	if ( ! class_exists( 'WooCommerce' ) ) {
		return;
	}
	 ...
	 */
	if ( ! apply_filters( 'creative_agency_load_woocommerce_styles', $needed ) ) {
		return;
	}

	wp_enqueue_style(
		'creative-agency-woocommerce',
		get_template_directory_uri() . '/assets/css/woocommerce.css',
		array( 'creative-agency-style' ),
		CREATIVE_AGENCY_VERSION
	);
}
add_action( 'wp_enqueue_scripts', 'creative_agency_woocommerce_styles', 20 );
 ...
 * @return int
 */
function creative_agency_woocommerce_columns() {
	return 3;
}
add_filter( 'loop_shop_columns', 'creative_agency_woocommerce_columns' );
