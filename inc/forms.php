<?php
/**
 * Form styling, for Creative Agency's own form and for whichever plugin a site uses.
 *
 ...
 * in the middle of a Creative Agency page looking like a different website, the theme
 ...
 * @return string[]
 */
function creative_agency_active_form_plugins() {
	$found = array();
 ...
 */
function creative_agency_enqueue_form_styles() {
	// The theme's own contact form is always a possibility, so the stylesheet
	// is not conditional on a plugin being present — it carries both.
	wp_enqueue_style(
		'creative-agency-forms',
		get_template_directory_uri() . '/assets/css/forms.css',
		array( 'creative-agency-style' ),
		CREATIVE_AGENCY_VERSION
	);
}
add_action( 'wp_enqueue_scripts', 'creative_agency_enqueue_form_styles' );

/**
 * Tell the editor about the same stylesheet, so a form block looks right there.
 */
function creative_agency_editor_form_styles() {
	add_editor_style( 'assets/css/forms.css' );
}
add_action( 'after_setup_theme', 'creative_agency_editor_form_styles', 20 );
